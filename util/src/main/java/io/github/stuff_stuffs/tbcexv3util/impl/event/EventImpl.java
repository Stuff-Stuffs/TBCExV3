...
import io.github.stuff_stuffs.tbcexv3util.api.util.event.InvokerFactory;
...
public class EventImpl<ViewType, EventType> implements Event<ViewType, EventType> {
    ...
    public EventType getInvoker() {
        if (invoker == null) {
            ...
            invoker = invokerFactory.createInvoker(values.toArray(i -> (EventType[]) Array.newInstance(key.mutClass(), i)), enter, exit);
        }
        return invoker;
    }
